...
fn main() -> anyhow::Result<()> {
    ...
    let temp = tempdir().with_context(|| "Failed to create temporary directory")?;
    let temp_dir = temp.path();
    let dest = temp_dir.join(".msixpack");

    // Download the windows toolkit if it's not found
    verify_toolkit()?;
    ...
    bundle_package(dest, "folio_x64.msix").with_context(|| "Failed to bundle package")?;

    Ok(())
}

fn verify_toolkit() -> anyhow::Result<()> {
    if !toolkit_exists()? {
...
fn data_dir() -> PathBuf {
    dirs::data_dir().unwrap().join("msixpack")
}
